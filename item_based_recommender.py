...
movie_name = pd.Series(user_movie_df.columns).sample(1).values[0] #rastgele bi filmi seçip eşleştirmek istersek
movie_name = user_movie_df[movie_name]
user_movie_df.corrwith(movie_name).sort_values(ascending=False).head(10)


def check_film(keyword, user_movie_df): #istenilen bir key veriliyor dataframe e gidiyor titlelarda dolanıyor ve içerisinde yer alan keywordün olduğu filmler geliyor.
